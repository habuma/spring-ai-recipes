...
public class ChatloopApplication {
  ...
  @Bean
  ApplicationRunner go(CompiledGraph<SupportState> compiledGraph) {
    return args -> {
      ...
      try (Scanner scanner = new Scanner(System.in)) {
        while (true) {
          ...
          var response = compiledGraph.invoke(Map.of("user_question", input));
          var resolution = response.get().getResolution();
